...
Next to the analysis function, you should add the following unit tests:

- A test for the top-level (backend agnostic) analysis function in `tests/analysis/test_analyses.py`.
- Tests for the backend-specific analysis implementations in `tests/<backend>/analysis/test_<analysis_module>.py`.
- Tests for the analysis results in `tests/analysis_results/test_<analysis_module>.py`. These tests should make use
  of reference data stored in `tests/_data/analysis_results`. To generate this reference data, add the analysis to
  the reference data generation script in `scripts/test_data/config.py`. New reference data can be generated by running
